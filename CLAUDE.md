...
| **NuMuon (arXiv:2603.03597)** | **~-0.003 to -0.008 bpb (est.)** | **FIRST-ADD for future competition — nuclear-norm constraint on Muon NS step steers weights toward low-rank structure during training; 55.9% more compressible under GPTQ → directly reduces post-quant val_bpb gap; drop-in Muon swap; ~10-20 lines; zero legality risk. Not tried in competition.** |
| **MELT (arXiv:2605.07721, May 2026)** | **unknown** | **WATCH — Memory-Efficient Looped Transformer; shares single KV cache per layer across loops via learnable gating; enables 4th-5th loop iteration at same memory budget; requires two-phase training (distillation from base model); high implementation complexity; may not fit 10-min constraint.** |
