...
# v10_safe: v9a base (11-gram backoff, entropy-adaptive) + Hedge Mixer + add-delta smoothing
# NO model scaling, NO ternary quant — safe fallback for v10 moonshot
class Hyperparameters:
    ...
    # TTT disabled in v10_safe — eval time used for n-gram + hedge mixer
...
def eval_val_sliding(
    ...
    N-gram: score-first/update-after (legal per competition rules).
    Hedge Mixer: multiplicative-weights online combination of neural + n-gram.
    Add-delta smoothing: (count + delta) / (ctx_count + delta * vocab_size)."""
    seq_len = eval_seq_len or args.train_seq_len
    ...
    if use_ngram:
        ...
        if rank == 0:
            print(
                f"v10_safe:ngram_cache:enabled orders={ngram_min_order}-{ngram_order} "
                f"entropy={ngram_entropy} delta={ngram_delta} "
                f"hedge={hedge_enabled} eta={hedge_eta} buckets={ngram_buckets}",
                flush=True,
            )

    # Hedge Mixer state: per-rank, global across eval (not reset per-window)
    # w_neural + w_ngram = 1.0 always; updated online via multiplicative weights
        ...
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for i, ws in enumerate(batch_ws):
                ...
                if use_ngram:
                    ...
                    # Multi-order backoff: highest order first, with add-delta smoothing
                    ...
                    for oi in range(_n_orders - 1, -1, -1):
                        ...
                        if needs_fill.any():
                            ...
                            # Add-delta smoothing: (count + delta) / (ctx + delta * vocab)
                    ...
                    # Apply Hedge Mixer or fixed entropy-adaptive alpha
                    ...
                    if has_match.any():
                        ...
                        if ngram_entropy and seg_ent is not None:
                            ...
                            # Order-adaptive center: 11-gram trusts at lower entropy
                            ent_center = 3.0
                            slope = 0.25
                        ...
                        if hedge_enabled:
                            # Hedge Mixer: replace static_alpha with adaptive weights
                            # w_neural and w_ngram are maintained globally across this eval
                            ...
                            # Update hedge weights using per-token multiplicative update
                            # (average losses over matched segment for stability)
                            ...
                            # Renormalize (keep sum = 1 for numerical stability)
                            w_total = hedge_w_neural + hedge_w_ngram
                    ...
                    # Update n-gram tables AFTER scoring (score-first protocol)
                    for oi in range(_n_orders):
                        if order_data[oi] is None:
                            continue
                        v_idx, ctx_key, full_key = order_data[oi]
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    return out
# --- TEST-TIME TRAINING (disabled in v10_safe, kept for TTT_ENABLED=1 override) ---
def _ttt_run_phase(
    model: nn.Module,
...
def main() -> None:
    ...
    if master_process:
        ...
        if quant_file_bytes + code_bytes > 16_000_000:
            log0(f"WARNING: artifact {quant_file_bytes + code_bytes} > 16MB LIMIT!")
        else:
            log0(f"artifact_ok: {quant_file_bytes + code_bytes} / 16000000 bytes")
    if distributed:
        dist.barrier()
