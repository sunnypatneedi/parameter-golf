...
def main() -> None:
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        # Temperature calibration sweep (zero extra training cost)
        for temp in [0.95, 0.96, 0.97, 0.98, 0.99]:
            tc_loss, tc_bpb = eval_val_sliding(
                args, eval_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                stride=args.eval_stride, eval_seq_len=sw_seq_len, temperature=temp,
            )
            log0(f"temp_cal T={temp:.2f} val_loss:{tc_loss:.4f} val_bpb:{tc_bpb:.4f}")
